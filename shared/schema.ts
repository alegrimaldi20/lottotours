...
export const lotteries = pgTable("lotteries", {
  ...
  drawId: text("draw_id").notNull(), // Unique draw identifier 
  numbersToSelect: integer("numbers_to_select").notNull().default(6), // How many numbers player must select
  totalNumbers: integer("total_numbers").notNull().default(49), // Total pool of numbers (1 to X)
  winningNumbers: text("winning_numbers").array(), // Array of winning number strings
  createdAt: timestamp("created_at").defaultNow(),
});
...
export const lotteryTickets = pgTable("lottery_tickets", {
  ...
  selectedNumbers: text("selected_numbers").array().notNull(), // Array of player's selected numbers
  isAutoGenerated: boolean("is_auto_generated").notNull().default(false), // True if numbers were auto-picked
