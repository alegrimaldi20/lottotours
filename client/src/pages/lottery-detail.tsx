...
export default function LotteryDetail() {
  ...
  const buyTickets = async () => {
    ...
    try {
      ...
      for (let index = 0; index < ticketCart.length; index++) {
        ...
        try {
          ...
          const response = await fetch(`/api/lotteries/${lottery.id}/purchase`, {
            ...
            body: JSON.stringify({
              selectedNumbers: ticket.selectedNumbers.map(n => Number(n)),
              isAutoGenerated: ticket.isAutoGenerated || false
            })
          });
          ...
          if (response.ok || response.status === 201) {
            ...
            // Small delay between tickets to show progress
          ...
          errors.push(`Ticket ${index + 1}: ${error.message || 'Connection error'}`);
      ...
      if (completedTickets > 0 && errors.length === 0) {
        ...
        setTimeout(() => setShowConfetti(false), 4000);
        toast({
          title: "Compra parcial exitosa",
          description: `${completedTickets} de ${ticketCart.length} tickets comprados. Revisa los detalles.`,
          variant: "default"
        });
        setTicketCart([]);
        queryClient.invalidateQueries({ queryKey: [`/api/lotteries/${params?.id}`] });
        ...
        toast({
          ...
          description: `No se pudo completar la compra: ${errors.join(', ')}`,
          variant: "destructive"
        });
        setTimeout(() => setPurchaseState('idle'), 3000);
      }
      ...
      toast({
        title: "Error en compra",
        description: `Error inesperado: ${error.message || 'Intenta nuevamente'}`,
        variant: "destructive"
      });
      setTimeout(() => setPurchaseState('idle'), 3000);
    }
