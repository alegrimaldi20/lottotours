...
export default function LotteryDetail() {
  ...
  const buyTickets = async () => {
    ...
    try {
      // Process all tickets using the correct endpoint
      ...
      for (let index = 0; index < ticketCart.length; index++) {
        ...
        try {
          ...
          const response = await fetch(`/api/lotteries/${lottery.id}/purchase`, {
            ...
            body: JSON.stringify({
              selectedNumbers: ticket.selectedNumbers.map(String),
      ...
      if (errors.length === 0) {
        ...
        setTicketCart([]);
        // Refresh the lottery data and user tokens
        ...
        setPurchaseState('error');
        toast({
          title: completedTickets > 0 ? "Compra parcial" : "Error en compra",
          description: completedTickets > 0 
            ? `${completedTickets} tickets comprados. Errores: ${errors.join(', ')}`
            : `No se pudo completar la compra: ${errors.join(', ')}`,
          variant: "destructive"
        });
      }
    } catch (error) {
      setBuyingTickets(false);
      setPurchaseState('error');
      console.error("General purchase error:", error);
      toast({
        title: "Error en compra",
        description: "Error inesperado durante la compra",
        variant: "destructive"
      });
    }
  };
          ...
          {purchaseState === 'processing' && (
            ...
            >
              ...
              >
                ...
                <h3 className="text-2xl font-bold text-gray-900 mb-2">Procesando Compra</h3>
                <p className="text-gray-600 mb-4">Comprando tus tickets de la suerte...</p>
          ...
          {purchaseState === 'success' && (
            ...
            >
              <motion.div
                ...
                className="bg-gradient-to-br from-emerald-50 to-teal-50 rounded-2xl p-8 max-w-md mx-4 text-center border-2 border-emerald-200 relative overflow-hidden"
              >
                ...
                >
                  <div className="flex items-center justify-center gap-2 text-emerald-700">
                    ...
                    <span className="font-semibold">{purchaseResults.completedTickets} Tickets Comprados</span>
                  </div>
                  <div className="flex items-center justify-center gap-2 text-emerald-600">
                    <Coins className="w-5 h-5" />
                    <span>{purchaseResults.totalCost} Kairos Tokens Gastados</span>
                  </div>
                  <motion.p
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
